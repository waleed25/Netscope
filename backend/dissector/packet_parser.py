"""
Normalize raw packets from tshark NDJSON (live) and scapy (pcap) into
a clean, LLM-friendly dict format.
...
# DNP3 application layer control codes
_DNP3_FC: dict[str, str] = {
    ...
    "30": "Abort File",
    "33": "Response",
    "34": "Unsolicited Response",
    "35": "Authenticate Request",
    "36": "Authenticate Error",
}

# OPC-UA service types (MessageType field)
...
# OPC-UA NodeId service identifiers (numeric)
_OPCUA_SERVICE: dict[str, str] = {
    ...
    "803": "Publish",
    "841": "CreateMonitoredItems",
    "771": "DeleteSubscription",
    "845": "DeleteMonitoredItems",
    "751": "ModifySubscription",
    "855": "ModifyMonitoredItems",
    "813": "Republish",
    "717": "TranslateBrowsePathsToNodeIds",
    "863": "SetTriggeringRequest",
}


def _first(val) -> str:
    """tshark ek fields are always lists — grab the first value."""
    if isinstance(val, list):
        return str(val[0]) if val else ""
    return str(val) if val is not None else ""


def parse_tshark_json(data: dict, index: int) -> dict:
    """
    Parse a tshark NDJSON packet (tshark -T ek) into a normalized dict.
    The 'layers' key holds all dissected fields.
    """
    layers_raw = data.get("layers", {})

    def f(key: str) -> str:
        return _first(layers_raw.get(key, ""))

    # Determine layers/protocols present
    protocols_str = f("frame_protocols")  # e.g. "eth:ethertype:ip:tcp:http"
    ...
    # Modbus TCP  (tshark -T ek uses dot→underscore for field names)
    modbus_fc   = (f("mbtcp_pdu_type") or f("modbus_func_code")
                   or f("mbtcp_func_code") or f("modbus_funccode"))
    ...
    if details.get("modbus_fc"):
        ...
        info = f"Modbus {fc_name}{unit}{ref}"
        ...
        info = f"DNP3 {fc_name}{d_src}{d_dst}"
    elif details.get("opcua_msg_type"):
        svc = details.get("opcua_service") or ""
        info = f"OPC-UA {details['opcua_msg_type']}" + (f" / {svc}" if svc else "")
    elif http_method:
        info = f"HTTP {http_method} {http_host}{http_uri}"
    elif http_code:
        info = f"HTTP {http_code}"
    elif tls_sni:
        info = f"TLS SNI: {tls_sni}"
    elif dns_query:
        info = f"DNS Query: {dns_query}"
    elif dns_resp:
        info = f"DNS Response: {dns_resp}"
    elif arp_src:
        info = f"ARP op={arp_op} {arp_src} → {arp_dst}"
    elif icmp_type:
        info = f"ICMP type={icmp_type} {src_ip} → {dst_ip}"
    else:
        info = f"{src_ip}:{src_port} → {dst_ip}:{dst_port}"

    # Detect high-level protocol
...
def _detect_protocol_from_list(protocols: list, details: dict, sport: str, dport: str) -> str:
    ...
    # ICS checks first — tshark layer names take priority
...
def _safe(obj: Any, *attrs: str, default: str = "") -> str:
...
def detect_protocol(pkt_dict: dict) -> str:
    ...
    # ICS first
    if "MBTCP" in layers_upper or "MODBUS" in layers_upper or details.get("modbus_fc") \
            or sport in _MODBUS_PORTS or dport in _MODBUS_PORTS:
        return "MODBUS"
    if "DNP3" in layers_upper or details.get("dnp3_fc") \
            or sport in _DNP3_PORTS or dport in _DNP3_PORTS:
        return "DNP3"
    if "OPCUA" in layers_upper or details.get("opcua_msg_type") \
            or sport in _OPCUA_PORTS or dport in _OPCUA_PORTS:
        return "OPC-UA"
...
def parse_pyshark_packet(pkt, index: int) -> dict:
    """Parse a live pyshark packet into a normalized dict."""
    try:
    ...
    # IP layer
    if hasattr(pkt, "ip"):
        result["src_ip"] = _safe(pkt.ip, "src")
        result["dst_ip"] = _safe(pkt.ip, "dst")

    # IPv6 layer
    if hasattr(pkt, "ipv6") and not result["src_ip"]:
        result["src_ip"] = _safe(pkt.ipv6, "src")
        result["dst_ip"] = _safe(pkt.ipv6, "dst")

    # TCP
    if hasattr(pkt, "tcp"):
        result["src_port"] = _safe(pkt.tcp, "srcport")
        result["dst_port"] = _safe(pkt.tcp, "dstport")
        result["details"]["tcp_flags"] = _safe(pkt.tcp, "flags")
        result["details"]["tcp_seq"] = _safe(pkt.tcp, "seq")

    # UDP
    if hasattr(pkt, "udp"):
        result["src_port"] = _safe(pkt.udp, "srcport")
        result["dst_port"] = _safe(pkt.udp, "dstport")

    # DNS
    if hasattr(pkt, "dns"):
        qry_name = _safe(pkt.dns, "qry_name")
        resp_name = _safe(pkt.dns, "resp_name")
    ...
    # HTTP
    if hasattr(pkt, "http"):
        method = _safe(pkt.http, "request_method")
        uri = _safe(pkt.http, "request_uri")
        host = _safe(pkt.http, "host")
        resp_code = _safe(pkt.http, "response_code")
    ...
    # TLS/SSL
    if hasattr(pkt, "tls"):
        sni = _safe(pkt.tls, "handshake_extensions_server_name")
    ...
    # ARP
    if hasattr(pkt, "arp"):
        result["src_ip"] = _safe(pkt.arp, "src_proto_ipv4")
        result["dst_ip"] = _safe(pkt.arp, "dst_proto_ipv4")
        result["info"] = f"ARP {_safe(pkt.arp, 'opcode')}"

    # Packet length
    try:
        result["length"] = int(pkt.length)
    except Exception:
        pass
...
def parse_scapy_packet(pkt, index: int) -> dict:
    """Parse a scapy packet (from pcap file) into a normalized dict."""
    ...
    if pkt.haslayer(TCP):
        result["src_port"] = str(pkt[TCP].sport)
        result["dst_port"] = str(pkt[TCP].dport)
        result["details"]["tcp_flags"] = str(pkt[TCP].flags)
    elif pkt.haslayer(UDP):
        result["src_port"] = str(pkt[UDP].sport)
        result["dst_port"] = str(pkt[UDP].dport)
    ...
    if pkt.haslayer(ARP):
        ...
        result["info"] = f"ARP op={pkt[ARP].op}"
    ...
    if has_http and pkt.haslayer(HTTPRequest):
        ...
        host = pkt[HTTPRequest].Host.decode("utf-8", errors="replace") if pkt[HTTPRequest].Host else ""
        result["details"]["http_method"] = method
    ...
    if has_http and pkt.haslayer(HTTPResponse):
        status = pkt[HTTPResponse].Status_Code.decode("utf-8", errors="replace") if pkt[HTTPResponse].Status_Code else ""
        result["details"]["http_response_code"] = status
        result["info"] = f"HTTP Response {status}"

    if pkt.haslayer(ICMP):
        result["info"] = f"ICMP type={pkt[ICMP].type}"
